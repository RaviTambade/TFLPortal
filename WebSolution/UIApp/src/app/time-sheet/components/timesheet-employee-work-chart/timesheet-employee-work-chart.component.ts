...
export class TimesheetEmployeeWorkChartComponent {
  ...
  getChartData() {
    this.workmgmtSvc
      .getEmployeeActivityWiseHours(this.employeeId,this.selectedInterval)
      .subscribe((res) => {
        ...
        this.workCategoryDetails.forEach((category, index) => {
          ...
          let obj = {
            label: this.getLabelName(category.label,index)/* "week"+ (index+1)*/,
            data: [
              category.userStory,
              category.task,
              category.clientCall,
  ...
  getLabelName(orignalLabel: string, index: number) {
    ...
    switch (this.selectedInterval) {
      case 'week':
        label = new Date(orignalLabel.slice(0,10).split('-').reverse().join('-')).toDateString().slice(0,10);
        break;
