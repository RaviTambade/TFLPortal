...
export class ProjectService {
 ...
 // sendData(data : any){
  ...
  getAllProjects():Observable<any>{
    let url = "http://localhost:5294/api/projects/getproject/";
  ...
  sendProject(data:any){
    ...
    switch(project){
      case "Online Meeting Sheduling":{
        let url = "http://localhost:5294/api/projects/getproject/"+project;
        this.http.get(url).subscribe((data) =>{
          console.log(data);
          this.subject.next({data,project});
        });
        break;
      }
      case "Online Interview Sheduling":{
        let url = "http://localhost:5294/api/projects/getproject/"+project;
        this.http.get(url).subscribe((data) =>{
          console.log(data);
          this.subject.next({data,project});
