...
CREATE TABLE team_member(team_member_id INT NOT NULL AUTO_INCREMENT PRIMARY KEY, team_id INT NOT NULL, CONSTRAINT fk_teamid FOREIGN KEY (team_id) REFERENCES team (team_id)ON UPDATE CASCADE  ON DELETE CASCADE ,role_id INT NOT NULL, CONSTRAINT fk_roleid FOREIGN KEY (role_id) REFERENCES role(role_id)ON UPDATE CASCADE  ON DELETE CASCADE,emp_id INT NOT NULL, CONSTRAINT fk_emp_Id4 FOREIGN KEY(emp_id) REFERENCES employees(emp_id) ON UPDATE CASCADE ON DELETE CASCADE);
CREATE TABLE project(proj_id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,proj_name VARCHAR(100),planned_startDate DATETIME,planend_endDate DATETIME,actual_startDate DATETIME NOT NULL,actual_endDate DATETIME NOT NULL,proj_desc VARCHAR(255),team_member_id INT NOT NULL, CONSTRAINT fk_teamMemberId FOREIGN KEY (team_member_id) REFERENCES team_member(team_member_id)ON UPDATE CASCADE  ON DELETE CASCADE,team_id INT NOT NULL, CONSTRAINT fk_teamId1 FOREIGN KEY (team_id) REFERENCES team(team_id)ON UPDATE CASCADE  ON DELETE CASCADE,emp_id INT NOT NULL, CONSTRAINT fk_empid2 FOREIGN KEY (emp_id) REFERENCES employees(emp_id)ON UPDATE CASCADE  ON DELETE CASCADE);
...
INSERT INTO project(proj_name,planned_startDate,planend_endDate,actual_startDate,actual_endDate,proj_desc,team_member_id,team_id ,emp_id)VALUES('Online Meeting Sheduling','2021-02-01','2021-03-01','2021-02-02','2021-03-03','Compeny requirement want to organize meetins online',1,1,1);
INSERT INTO project(proj_name,planned_startDate,planend_endDate,actual_startDate,actual_endDate,proj_desc,team_member_id,team_id,emp_id)VALUES('Online Interview Sheduling','2022-05-10','2022-05-12','2022-05-10','2022-05-13','We want to argent hiring of new employeess for new projects',2,2,2);
INSERT INTO project(proj_name,planned_startDate,planend_endDate,actual_startDate,actual_endDate,proj_desc,team_member_id,team_id,emp_id)VALUES('MockTesting Sheduling','2021-02-01','2021-03-01','2021-02-02','2021-03-03','Compeny requirement want to organize meetins online',3,3,3);
...
SELECT  project.proj_name, project.actual_startDate,project.actual_endDate,project.proj_desc, team.team_id,employees.empfirst_name,employees.emplast_name
FROM project
INNER JOIN team ON team.team_id = project.team_id
INNER JOIN employees ON employees.emp_id = project.emp_id;
